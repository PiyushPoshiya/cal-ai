...
actor CachedImageManager {
    ...
    @discardableResult
    func requestImage(for asset: PhotoAsset, targetSize: CGSize, completion: @escaping ((image: Image?, uiImage: UIImage?, isLowerQuality: Bool)?) -> Void) -> PHImageRequestID? {
        ...
        let requestID = imageManager.requestImage(for: phAsset, targetSize: targetSize, contentMode: imageContentMode, options: requestOptions) { image, info in
            if let error = info?[PHImageErrorKey] as? Error {
                ...
                let result = (image: Image(uiImage: image), uiImage: image, isLowerQuality: isLowerQualityImage)
                completion(result)
            } else {
                completion(nil)
            }
